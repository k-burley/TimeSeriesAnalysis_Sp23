...
library(lubridate)
library(ggplot2)
library(forecast) # gives you autoplot, which LL prefers for time series plots
library(Kendall)
library(tseries)
...
raw_inflow_data <- read.table(file="C:/Users/12256/OneDrive - University of North Carolina at Chapel Hill/Documents/GitHub/TimeSeriesAnalysis_Sp23_RVersion/Data/inflowtimeseries.txt",header=FALSE,skip=0)
...
# Outlier detection will typically look at deviation from the mean
...
plot(decompose_inflow_data)
  # a couple residuals in the random component are quite high
  # we can't really know if they are outliers are not just visually
...
sd_inflow <- sd(inflow_random)
  # Just as an example, we will run missing data/outlier detection on only the random component
...
ggplot(inflow_random, aes(x=month, y=inflow)) +
            geom_boxplot()
  # More variation in rainy season than dry season
...
# Overall - suspicious we might have outliers in the data
```
...
outlier(inflow_random$inflow)
  # returns the highest value we have in the series
...
print(chi_test) #look at the p-value to find the decision 
  # reject null hypothesis, alternative hypothesis: this value is an outlier
...
#Since we removed seasonality replacing with overall mean instead of local mean is acceptable
  # replace these values with the local mean

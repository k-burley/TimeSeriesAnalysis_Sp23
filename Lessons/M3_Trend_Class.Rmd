...
nobs <- nrow(raw_inflow_data) # Important because sometimes we'll want to run for all hydros and know observations

#If your file does not have header like this one you can add column names after creating the data frame
colnames(raw_inflow_data)=c("Month","Year", "HP1", "HP2","HP3") # HP = Hydro Plant

#Checking data
head(raw_inflow_data)
str(raw_inflow_data)
```
## Date or time period - will always need to do this in time series!
...
my_date <- my(my_date)  #function my from package lubridate (my = month year) - use the order that the elements are appearing
  # can also use as.date, but you have to specify formatting
  # lubridate does the formatting for you
head(my_date) # assumes that its the first day of each month, that is ok.
...
format(my_date[1],format="%Y-%m")
format(my_date[1],format="%Y-%b") # Can always change the formatting if you would like. Can be useful for plotting
...
hist(inflow_data[,"HP1"],breaks=30,col="gray")  #increase number of bars with breaks input
  # Helpful to see distribution
  # Inflow data usually follow log-normal distribution
  # Left skewed because no negative observations


#Graph 4: Scatter Plot of HP1 and HP2
plot(inflow_data[,"HP1"],inflow_data[,"HP2"]) # strong, linear correlation between HP1 and HP2
  # These reservoirs are close together, so we expect a strong correlation
```
...
ts_inflow_data <- ts(inflow_data[,2:(2+nhydro-1)]) #note that we are only transforming columns with inflow data, not the date columns
  # Can also use $ dataframe syntax
ts_inflow_data
#plot(ts_inflow_data[,1])
acf(ts_inflow_data[,1]) # Will always range between (-1,1)
  # We can see a seasonal component here
pacf(ts_inflow_data[,1]) # Will always range between (-1,1)
...
## Trend Exploration in Class 1/31/23
```{r}
inflow_data <- inflow_data %>%
  mutate(year=as.numeric(format(my_date,'%Y'))) %>%
  filter(year!=2011)

...
beta1 <- linear_trend$coefficients[2] # coefficient

detrend_inflow_data <- inflow_data[,(iHP+1)]-(beta0+beta1*t)
   # can also use residuals
# detrend_inflow_data <- residuals(linear_trend)
...
# Want to use the detrended dataset
iHP=1
...
```{r}
# Try for all columns in the data except for just the first one!
...
SMKtest <- SeasonalMannKendall(ts_inflow_data[,iHP])
  # tau = 0.175, 2-sided pvalue =1.5543e-15
  # Reject null hypotheses that series is stationary
...
# No version that can handle seasonality! 
...
print(adf.test(ts_inflow_data[,iHP]))
